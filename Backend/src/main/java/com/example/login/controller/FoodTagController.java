...
public class FoodTagController {
    ...
    @GetMapping
    public ResponseEntity<List<FoodTagDTO>> getTags(@RequestParam Long userId) {
        return ResponseEntity.ok(service.getTagsByUser(userId));
    }

    // POST /api/users/food-tags
    @PostMapping
    public ResponseEntity<FoodTagDTO> addTag(@RequestBody FoodTagDTO dto) {
        return ResponseEntity.ok(service.addTag(dto));
    }

    // PUT /api/users/food-tags/{id}
    @PutMapping("/{id}")
    public ResponseEntity<FoodTagDTO> updateTag(@PathVariable Long id, @RequestBody FoodTagDTO dto) {
